...
class objective_function(ABC):
    def __init__(
        self, name="template", use_polarity=True, has_derivative=True, default_blur=1.0
    ):
        self.name = name
...
def get_iwe(
    ...
    iwe, iwe_drv = events_to_image_drv(
        xs, ys, ps, jx, jy, interpolation="bilinear", compute_gradient=compute_gradient
    )
    return iwe, iwe_drv
...
class variance_objective(objective_function):
    ...
    def evaluate_gradient(
        ...
        for grad_dim in range(d_iwe.shape[0]):
            ...
            gradient.append(
                np.mean(zero_mean * (d_iwe[grad_dim] - np.mean(d_iwe[grad_dim])))
            )
        grad = np.array(gradient)
        return -grad
