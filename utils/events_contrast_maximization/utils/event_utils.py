...
def interpolate_to_derivative_img(pxs, pys, dxs, dys, d_img, w1, w2):
    ...
    for i in range(d_img.shape[0]):
        d_img[i].index_put_(
            (pys, pxs), w1[i] * (-(1.0 - dys)) + w2[i] * (-(1.0 - dxs)), accumulate=True
        )
        d_img[i].index_put_(
            (pys, pxs + 1), w1[i] * (1.0 - dys) + w2[i] * (-dxs), accumulate=True
        )
        d_img[i].index_put_(
            (pys + 1, pxs), w1[i] * (-dys) + w2[i] * (1.0 - dxs), accumulate=True
        )
        d_img[i].index_put_(
...
def events_to_image_drv(
    ...
    if compute_gradient:
        jacobian_x, jacobian_y = torch.from_numpy(jacobian_xn), torch.from_numpy(
            jacobian_yn
        )
...
def events_to_image_torch(
    ...
    img = torch.zeros(img_size, dtype=ps.dtype).to(device)
    if (
        interpolation == "bilinear"
        and xs.dtype is not torch.long
...
def events_to_image_torch_iros(
    xs,
    ys,
    ps,
    device=None,
    ...
    img = torch.zeros(img_size, dtype=xs.dtype).to(device)
    if (
        interpolation == "bilinear"
        and xs.dtype is not torch.long
        and ys.dtype is not torch.long
...
def events_to_voxel_torch(
    ...
    zeros = torch.zeros(t_norm.size()).to(device)
    for bi in range(B):
...
def events_to_voxel_timesync_torch_iros(
    xs,
    ys,
    ts,
    ps,
    ...
    end_idx = np.searchsorted(np_ts, t1)
    assert start_idx < end_idx
    voxel = events_to_voxel_torch_iros(
        xs[start_idx:end_idx],
        ys[start_idx:end_idx],
        ts[start_idx:end_idx],
        ps[start_idx:end_idx],
...
def events_to_voxel_torch_iros(
    xs, ys, ts, ps, B, device=None, sensor_size=(180, 240), temporal_bilinear=True
):
    """
    Turn set of events to a voxel grid tensor, using temporal bilinear interpolation
    ...
    dt = ts[-1] - ts[0]
    t_norm = (ts - ts[0]) / dt * (B - 1)
    zeros = torch.zeros(t_norm.size(), dtype=xs.dtype).to(device)
    for bi in range(B):
        if temporal_bilinear:
            bilinear_weights = torch.max(zeros, 1.0 - torch.abs(t_norm - bi))
            weights = ps * bilinear_weights
            vb = events_to_image_torch_iros(
                xs,
                ys,
                weights,
                device,
...
def events_to_voxel(xs, ys, ts, ps, B, sensor_size=(180, 240), temporal_bilinear=True):
    ...
    for bi in range(B):
        if temporal_bilinear:
            ...
            vb = events_to_image(
                xs[beg:end], ys[beg:end], weights[beg:end], sensor_size=sensor_size
            )
        vb = events_to_image(
            xs, ys, weights.squeeze(), sensor_size=sensor_size, interpolation=None
        )
        bins.append(vb)
    bins = np.stack(bins)
    return bins
...
if __name__ == "__main__":
    ...
    print(
        "conversion to torch: time elapsed  = {:0.5f}".format((end - start) / test_loop)
    )
