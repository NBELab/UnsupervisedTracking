...
def get_rosbag_stats(bag, event_topic, image_topic=None, flow_topic=None):
    ...
    for topic_name, topic_info in topics.items():
        if topic_name == event_topic:
            ...
            print(
                "Found events topic: {} with {} messages".format(
                    topic_name, topic_info.message_count
                )
            )
        if topic_name == image_topic:
            num_img_msgs = topic_info.message_count
            print(
                "Found image topic: {} with {} messages".format(
                    topic_name, num_img_msgs
                )
            )
        if topic_name == flow_topic:
            num_flow_msgs = topic_info.message_count
            print(
                "Found flow topic: {} with {} messages".format(
                    topic_name, num_flow_msgs
                )
            )
    ...
    with rosbag.Bag(rosbag_path, "r") as bag:
        ...
        for topic, msg, t in bag.read_messages():
            ...
            if topic == image_topic:
                timestamp = timestamp_float(msg.header.stamp) - (
                    first_ts if zero_timestamps else 0
                )
                if is_color:
                ...
                event_msg_sum += 1
                if (
                    event_msg_sum % num_msgs_between_logs == 0
                    or event_msg_sum >= num_event_msgs - 1
                ):
                    print(
                        "Event messages: {} / {}".format(
                            event_msg_sum + 1, num_event_msgs
                        )
                    )
                for e in msg.events:
                    ...
                    if e.polarity:
                        ...
                        num_neg += 1
                    last_ts = timestamp
                if (len(xs) > max_buffer_size and timestamp >= start_time) or (
                    end_time is not None and timestamp >= start_time
                ):
                    ...
                    if (
                        ...
                        print(
                            "Sensor size inferred from events as {}".format(sensor_size)
                        )
        ...
        ep.add_metadata(
            num_pos, num_neg, last_ts - t0, t0, last_ts, img_cnt, flow_cnt, sensor_size
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path", help="ROS bag file to extract or directory containing bags"
    )
    parser.add_argument(
        "--output_dir",
    ...
    parser.add_argument(
        "--flow_topic",
        default=None,
        help="Flow topic (if left empty, no flow will be collected)",
    )
    parser.add_argument(
        "--zero_timestamps",
        action="store_true",
    ...
    parser.add_argument(
        "--is_color",
        action="store_true",
        help="Set flag to save frames from image_topic as 3-channel, bgr color images",
    )
