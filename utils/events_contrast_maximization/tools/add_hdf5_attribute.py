...
def get_filepaths_from_path_or_file(
    path, extensions=[], datafile_extensions=[".txt", ".csv"]
):
    files = []
    path = path.rstrip("/")
...
def add_attribute(h5_filepaths, group, attribute_name, attribute_value, dry_run=False):
    for h5_filepath in h5_filepaths:
        print(
            "adding {}/{}[{}]={}".format(
                h5_filepath, group, attribute_name, attribute_value
            )
        )
        if dry_run:
            continue
...
if __name__ == "__main__":
    ...
    required.add_argument(
        "--path",
        help="Can be either 1: path to individual hdf file, "
        + "2: txt file with list of hdf files, or "
        + "3: directory (all hdf files in directory will be processed).",
        required=True,
    )
    ...
    optional.add_argument(
        "--group",
        help="Group to add attribute to. Subgroups "
        + "are represented like paths, eg: /group1/subgroup2...",
        default="",
    )
    optional.add_argument(
        "--dry_run",
