...
def get_sensor_size(txt_path):
    try:
        ...
        sensor_size = None
        print(
            "Warning: could not read sensor size from first line of {}".format(txt_path)
        )
    return sensor_size


def extract_txt(txt_path, output_path, zero_timestamps=False, packager=hdf5_packager):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path",
        help="txt file to extract or directory containing txt files",
        default="D:/siamEvents/real_event_dataset/events",
    )
    parser.add_argument(
        "--output_dir",
        default="D:/siamEvents/real_event_dataset/events/bags",
    ...
    if os.path.isdir(args.path):
        txt_paths = sorted(
            list(glob.glob(os.path.join(args.path, "*.txt")))
