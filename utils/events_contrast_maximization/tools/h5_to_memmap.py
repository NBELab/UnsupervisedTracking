...
def save_additional_data_as_mmap(f, mmap_pth, data):
    ...
    print(
        "Writing {} to mmap {}, timestamps to {}".format(
            data_key, data_path, data_ts_path
        )
    )
    h, w, c = 1, 1, 1
    ...
    mmp_img_ts = np.memmap(
        data_ts_path, dtype="float64", mode="w+", shape=(num_data, 1)
    )
    mmp_event_indices = np.memmap(
        data_event_idx_path, dtype="uint16", mode="w+", shape=(num_data, 1)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", default=None, help="Path to extract (same as bag if left empty)"
    )
    parser.add_argument(
        "--not_overwrite",
        action="store_false",
        help="If set, will not overwrite\
