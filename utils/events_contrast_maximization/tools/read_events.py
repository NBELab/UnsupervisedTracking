...
def read_memmap_events(
    memmap_path,
    skip_frames=1,
    return_events=False,
    images_file="images.npy",
    images_ts_file="timestamps.npy",
    optic_flow_file="optic_flow.npy",
    optic_flow_ts_file="optic_flow_timestamps.npy",
    events_xy_file="xy.npy",
    events_p_file="p.npy",
    events_t_file="t.npy",
):
    assert os.path.isdir(memmap_path), (
        "%s is not a valid memmap_pathectory" % memmap_path
    )
    ...
    for subroot, _, fnames in sorted(os.walk(memmap_path)):
            ...
            if fname.endswith(".npy"):
                if fname == "index.npy":  # index mapping image index to event idx
                    ...
                    indices = indices.astype(
                        "int64"
                    )  # ignore event indices which are 0 (before first image)
        ...
        if len(data) > 0:
            ...
            if not (
                len(data["p"]) == len(data["xy"]) and len(data["p"]) == len(data["t"])
            ):
