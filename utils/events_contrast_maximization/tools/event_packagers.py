...
class packager:
    ...
    @abstractmethod
    def add_metadata(
        self, num_events, num_pos, num_neg, duration, t0, tk, num_imgs, num_flow
    ):
        pass
...
class hdf5_packager(packager):
    ...
    def package_image(self, image, timestamp, img_idx):
        image_dset = self.events_file.create_dataset(
            "images/image{:09d}".format(img_idx), data=image, dtype=np.dtype(np.uint8)
        )
        image_dset.attrs["size"] = image.shape
        image_dset.attrs["timestamp"] = timestamp
        image_dset.attrs["type"] = (
            "greyscale"
    ...
    def package_flow(self, flow_image, timestamp, flow_idx):
        flow_dset = self.events_file.create_dataset(
            "flow/flow{:09d}".format(flow_idx),
            data=flow_image,
        ...
        for datatype in datatypes:
            if datatype in self.events_file.keys():
                ...
                for image in self.events_file[datatype]:
                    ...
                    self.events_file[datatype][image].attrs["event_idx"] = (
                        event_idx + added
                    )

    def add_metadata(
        self, num_pos, num_neg, duration, t0, tk, num_imgs, num_flow, sensor_size
    ):
