...
def cxy_wh_2_rect1_torch(pos, sz):
    return torch.tensor([pos[0]-sz[0]/2+1, pos[1]-sz[1]/2+1, sz[0], sz[1]])  # 1-index

def bb_2_cxy_wh(bb):
    return np.array([bb[0],bb[1]]), np.array([bb[2]-bb[0],bb[3]-bb[1]])

def bbox_2_cxy_wh(bb):
    return np.array([bb[0]+(bb[2]-bb[0])/2,bb[1]+(bb[3]-bb[1])/2]), np.array([bb[2]-bb[0],bb[3]-bb[1]])
...
def rect1_2_cxy_wh_test(rect):
    return np.array([rect[0]+rect[2], rect[1]+rect[3]]), np.array([rect[2], rect[3]])  # 0-index
def rect1_2_cxy_wh_torch(rect,device):
    return torch.tensor([rect[0]+rect[2]/2-1, rect[1]+rect[3]/2-1]).to(device), torch.tensor([rect[2], rect[3]]).to(device)  # 0-index
...
def cxy_wh_2_bbox_torch(cxy, wh):
    return torch.tensor([cxy[0]-wh[0]/2, cxy[1]-wh[1]/2, cxy[0]+wh[0]/2, cxy[1]+wh[1]/2])  # 0-index
...
def crop_chw_torch(image, bbox, out_sz,device, padding=(0, 0, 0)):
    ...
    d = -b * bbox[0]
    #print(a,b,c,d)
    mapping = torch.tensor([[a, 0, c],[0, b, d]]).type(torch.float32).unsqueeze(0).to(device)
    image = image.unsqueeze(0)
    x = kornia.geometry.transform.warp_affine(image,mapping,(out_sz,out_sz),padding_mode="border", fill_value=padding)
    return x.squeeze()
