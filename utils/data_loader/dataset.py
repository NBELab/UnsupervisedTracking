...
class BaseVoxelDataset(Dataset):
    ...
    def __init__(
        ...
        if (
            self.sensor_resolution is None
            or self.has_flow is None
            or self.t0 is None
            or self.tk is None
            or self.num_events is None
            or self.frame_ts is None
            or self.num_frames is None
        ):
            raise Exception(
                "Dataloader failed to intialize all required members ({})".format(
                    self.data_path
                )
            )
        ...
        if filter_hot_events:
            ...
            hot_num = min(
                self.find_ts_index(secs_for_hot_mask + self.t0), self.num_events
            )
            xs, ys, ts, ps = self.get_events(0, hot_num)
            self.hot_events_mask = get_hot_event_mask(
                xs.astype(np.int),
                ys.astype(np.int),
                ps,
                self.sensor_resolution,
                num_hot=int(self.num_pixels * hot_pix_percent),
            )
            self.hot_events_mask = np.stack(
                [self.hot_events_mask] * self.channels, axis=2
            ).transpose(2, 0, 1)
        else:
        ...
        for norm in ["RobustNorm", "LegacyNorm"]:
            if norm in transforms.keys():
                vox_transforms_list = [
                    eval(t)(**kwargs) for t, kwargs in transforms.items()
                ]
                del transforms[norm]
    ...
    def __getitem__(self, index, seed=None):
        ...
        ), "index {} out of bounds (0 <= x < {})".format(index, self.__len__())
        ...
        if len(xs) < 3:
            ...
            if self.neg_events:
                ps = torch.from_numpy(ps.astype(np.float32)) * -1
            else:
                ps = torch.from_numpy(ps.astype(np.float32))
            voxel = self.get_voxel_grid(
                xs, ys, ts, ps, combined_voxel_channels=self.combined_voxel_channels
            )
    ...
    def compute_timeblock_indices(self):
        ...
        for i in range(self.__len__()):
            start_time = (
                (self.voxel_method["t"] - self.voxel_method["sliding_window_t"]) * i
            ) + self.t0
    ...
    def set_voxel_method(self, voxel_method):
        ...
        if self.voxel_method["method"] == "k_events":
            self.length = max(
                int(
                    self.num_events
                    / (voxel_method["k"] - voxel_method["sliding_window_w"])
                ),
                0,
            )
            ...
            self.length = max(
                int(
                    self.duration
                    / (voxel_method["t"] - voxel_method["sliding_window_t"])
                ),
                0,
            )
            ...
            raise Exception(
                "Invalid voxel forming method chosen ({})".format(self.voxel_method)
            )
        if self.length == 0:
            raise Exception(
                "Current voxel generation parameters lead to sequence length of zero"
            )
    ...
    def get_event_indices(self, index):
        ...
        if not (idx0 >= 0 and idx1 <= self.num_events):
            raise Exception(
                "WARNING: Event indices {},{} out of bounds 0,{}".format(
                    idx0, idx1, self.num_events
                )
            )
        return idx0, idx1
    ...
    def get_voxel_grid(self, xs, ys, ts, ps, combined_voxel_channels=True):
        ...
        if combined_voxel_channels:
            ...
            voxel_grid = events_to_voxel_torch(
                xs, ys, ts, ps, self.num_bins, sensor_size=self.sensor_resolution
            )
        else:
            # generate voxel grid which has size 2*self.num_bins x H x W
            voxel_grid = events_to_neg_pos_voxel_torch(
                xs, ys, ts, ps, self.num_bins, sensor_size=self.sensor_resolution
            )
...
class DynamicH5Dataset(BaseVoxelDataset):
    ...
    def load_data(self, data_path):
        ...
        print("working on ={}".format(data_path))
        ...
        self.frame_ts = []
        if "images" in self.h5_file.keys():
            for img_name in self.h5_file["images"]:
                self.frame_ts.append(
                    self.h5_file["images/{}".format(img_name)].attrs["timestamp"]
                )
...
class MemMapDataset(BaseVoxelDataset):
    ...
    def load_data(
        self,
        data_path,
        timestamp_fname="timestamps.npy",
        image_fname="images.npy",
        optic_flow_fname="optic_flow.npy",
        optic_flow_stamps_fname="optic_flow_stamps.npy",
        t_fname="t.npy",
        xy_fname="xy.npy",
        p_fname="p.npy",
    ):
        ...
        for subroot, _, fnames in sorted(os.walk(data_path)):
            ...
            if len(data) > 0:
                ...
                if "t" not in data:
                    ...
                    continue
                assert len(data["p"]) == len(data["xy"]) and len(data["p"]) == len(
                    data["t"]
                )
    ...
    def infer_resolution(self):
        if len(self.filehandle["images"]) > 0:
            ...
            print(
                "WARNING: sensor resolution inferred from maximum event coordinates - highly not recommended"
            )
            return [
                np.max(self.filehandle["xy"][:, 1]) + 1,
...
class SequenceDataset(Dataset):
    ...
    def __init__(
        ...
        proba_neg_events=0.0,
        proba_rev_events=0.0,
        proba_fast_back=0.0,
        random_search=False,
        ...
        self.proba_neg_events = proba_neg_events
        self.proba_rev_events = proba_rev_events
        self.proba_fast_back = proba_fast_back
    ...
    def __getitem__(self, i):
        ...
        u = np.random.rand()
        if u < self.proba_neg_events:
            self.dataset.neg_events = True
        ...
        # reverse events
        u = np.random.rand()
        if u < self.proba_rev_events:
            sequence = list(reversed(sequence))

        # fast back events
        u = np.random.rand()
        if u < self.proba_fast_back:
            rev = list(reversed(sequence))
            mid = len(sequence) // 2 + 1
            sequence[mid:] = rev[mid:]
        else:
            if self.random_search:
                indxs = np.array(random.sample(range(len(sequence) - 1), 2)) + 1
                sequence = [sequence[0], sequence[indxs[0]], sequence[indxs[1]]]
        self.dataset.neg_events = False
        return sequence
