...
def temporal_consistency_loss(
    image0, image1, processed0, processed1, flow01, alpha=50.0, output_images=False
):
    ...
    xx, yy = torch.meshgrid(
        torch.arange(t_width), torch.arange(t_height)
    )  # xx, yy -> WxH
    ...
    tc_map = (
        visibility_mask
        * torch.abs(processed1 - processed0_warped_to1)
...
def voxel_warping_flow_loss(
    voxel, displacement, output_images=False, reverse_time=False
):
    """Adapted from:
    Temporal loss, as described in Eq. (2) of the paper 'Learning Blind Video Temporal Consistency',
    Lai et al., ECCV'18.
    ...
    voxel_grid_warped = torch.zeros(
        (v_shape[0], 1, t_height, t_width), dtype=voxel.dtype, device=voxel.device
    )
    for i in range(t_channels):
        warp_magnitude_ratio = (
            (1.0 - i * displacement_increment)
            if reverse_time
            else i * displacement_increment
        )
        ...
        warping_grid = torch.stack(
            [warping_grid_y, warping_grid_x], dim=3
        )  # 1 x H x W x 2
        # Normalize the warping grid to between -1 and 1 (necessary for grid_sample API)
        warping_grid[:, :, :, 0] = (2.0 * warping_grid[:, :, :, 0]) / (
            t_height - 1
        ) - 1.0
        warping_grid[:, :, :, 1] = (2.0 * warping_grid[:, :, :, 1]) / (
            t_width - 1
        ) - 1.0
    ...
    tc_loss = -variance
    if not reverse_time:
        reverse_tc_loss = voxel_warping_flow_loss(
            voxel, displacement, output_images=False, reverse_time=True
        )
