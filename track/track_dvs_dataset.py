...
from events_contrast_maximization.utils.event_utils import events_to_voxel_timesync_torch
...
def test_events(net,files,data,bb_gt,num_bins,test_name):
    ...
    for video_id, video in tqdm(enumerate(files)):
        ...
        for k, indxs in enumerate(data[files[video]["name"]]["frames"]):
            ...
            bb_index = 0
            start_i = indxs[0]
            im = events_to_voxel_timesync_torch(xs,ys,ts,ps,num_bins,t0=frame_timestamp[start_i][0],t1=frame_timestamp[start_i][1],sensor_size=size,device="cpu")
            im = im.permute(1,2,0)
            ...
            for i in range(start_i + 1,indxs[1]):  # track
                try:
                    im = events_to_voxel_timesync_torch(xs,ys,ts,ps,num_bins,t0=frame_timestamp[i][0],t1=frame_timestamp[i][1],sensor_size=size,device="cpu")
                except Exception as e:
                    break
                im = im.permute(1,2,0)
                im2 = im.cpu().detach().numpy().astype('float32')
                for j in range(config.num_scale):  # crop multi-scale search region
                    window_sz = target_sz * (config.scale_factor[j] * (1 + config.padding))
                    bbox = cxy_wh_2_bbox(target_pos, window_sz)
                ...
                r_max, c_max = np.unravel_index(idx[best_scale].cpu().detach(), config.net_input_size)
...
def test_add_events_frames(net,files,data,bb_gt,num_bins,test_name):
    ...
    for video_id, video in tqdm(enumerate(files)):  # run without resetting
        ...
        for k, indxs in enumerate(data[files[video]["name"]]["frames"]):
            ...
            net.update(torch.Tensor(np.expand_dims(target_f, axis=0)).cuda(),torch.Tensor(np.expand_dims(target, axis=0)).cuda())
            res = [cxy_wh_2_rect1(target_pos, target_sz)]  # save in .txt
            patch_crop = np.zeros((config.num_scale, patch.shape[0], patch.shape[1], patch.shape[2]), np.float32)
            patch_crop_f = np.zeros((config.num_scale, target_f.shape[0], target_f.shape[1], target_f.shape[2]), np.float32)
            for i in range(start_i + 1,indxs[1]):  # track
                ...
                response = net(torch.Tensor(search_f).cuda(),torch.Tensor(search).cuda())
                peak, idx = torch.max(response.view(config.num_scale, -1), 1)
                peak = peak.data.cpu().numpy() * config.scale_penalties
                best_scale = np.argmax(peak)
                r_max, c_max = np.unravel_index(idx[best_scale].cpu().detach(), config.net_input_size)
...
def test_frames(net,files,data,bb_gt,test_name):
    ...
    for video_id, video in tqdm(enumerate(files)):  # run without resetting
        ...
        for k, indxs in enumerate(data[files[video]["name"]]["frames"]):
            ...
            net.update(torch.Tensor(np.expand_dims(target, axis=0)).cuda(), events = False)

            res = [cxy_wh_2_rect1(target_pos, target_sz)]  # save in .txt
            patch_crop = np.zeros((config.num_scale, target.shape[0], target.shape[1], target.shape[2]), np.float32)
            for i in range(start_i+1, indxs[1]):  # track
                try:
                    im = frames[i].squeeze()
                except:
                ...
                net.update(torch.Tensor(np.expand_dims(target, axis=0)).cuda(), events = False, lr=config.interp_factor)
...
def eval_auc(files,data,bb_gt, tracker_reg='S*', start=0, end=1e6, ev=0):
    ...
    if(ev == 0):
        trackers = glob.glob(fullfile('track','result',dataset, 'events', tracker_reg))
    elif(ev == 1):
        trackers = glob.glob(fullfile('track','result',dataset, 'frames', tracker_reg))
    else:
        trackers = glob.glob(fullfile('track','result',dataset, 'event_frames', tracker_reg))
    ...
    for i in range(n_seq):
        ...
        try:
            ...
            continue
        gt_rect = np.array([cxy_wh_2_rect1(*bbox_2_cxy_wh(x)) for x in bb_seq]).astype(np.float32)[:]
...
def parse_args():
    ...
    argparser.add_argument("--weights", default="./models/model.pth.tar", help="Path to model weights (default: ./models/model.pth.tar)")
