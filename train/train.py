...
def get_response(template_feat,search1_feat,label,initial_y):
    ...
    fake_yf = torch.fft.rfft2(torch.Tensor(fake_y).view(batch_size*gpu_num, 1, config.output_sz, config.output_sz).cuda())
...
def validate(val_loader):
    ...
    with torch.no_grad():
        ...
        for i, x in enumerate(val_loader):
            ...
            # consistency loss. target is the initial Gaussian label
            loss = (0.5*criterion(output, target) +0.2*criterion(output_e, target) + 0.3*criterion(output_f, target))/(batch_size * gpu_num)
...
def parse_args():
    ...
    parser.add_argument('-c', '--config', default=os.getcwd()+"/train/config/config.json", type=str,
                    help='config file path (default: CWD +/train/config/config.json)')
    parser.add_argument('-r', '--resume', default=None, type=str,
                    help='path to latest checkpoint (default: None)')
    parser.add_argument('-d', '--device', default=None, type=str,
                        help='indices of GPUs to enable (default: all)')
    parser.add_argument('--limited_memory', default=False, action='store_true',
...
def init_globals(args):
    global num_bins
    num_bins = args.config['data_loader']['args']['sequence_kwargs']['dataset_kwargs']['num_bins']
    ...
    global optimizer
    optimizer = torch.optim.SGD(model.parameters(), args.config['trainer']['lr'],
...
def main(args):
    ...
    save_path = os.path.join(args.config['trainer']['save_dir'], '{}_{:d}'.format(dt_string, num_bins))
